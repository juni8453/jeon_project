...
//		String path = "C:/Users/l9-morning/Documents/lcom_test/src/vue-spring-jeon/public/homeimages/";
		...
		String filename = dateresult + multipartFile.getOriginalFilename();
		String ext = filename.substring(filename.lastIndexOf(".")+1);
...
//			homeimg.sethiName(filename);
			homeimg.sethiName(filename);
			productService.insertHomeImg(homeimg);
	...
	@PostMapping("/deletehomeimg")
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	public ResponseEntity<?> deletehomeimg(@RequestBody HomeImg homeimg){
		productService.deleteHomeImg(homeimg);
