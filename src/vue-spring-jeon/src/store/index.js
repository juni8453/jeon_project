...
export default new Vuex.Store({
  ...
  actions: {
    loginProcess({ commit, state }, payload) {
      // let dev = 'localhost'
      // let sev = '3.38.87.14'
      console.log(payload)
  ...
  admin({commit,state}, payload) {
    return new Promise((resolve, reject) => {
      ...
      axios.get(`http://${state.dev}/api/admin/adminPage/${payload.page}`)
          .then(Response => {
            console.log(payload) 
            console.log(Response.data)
  ...
  UnpackToken({commit,state}) { // 새로고침 방지 메서드
    return new Promise((resolve, reject) => {
      axios.defaults.headers.common['Authorization'] = `Bearer ${localStorage.getItem("token")}`
      // localStorage에 저장된 token 사용
      axios.get(`http://${state.dev}/api/auth/unpackToken`)
          .then(Response => {
            console.log(Response.data)
